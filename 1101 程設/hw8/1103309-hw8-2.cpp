...
void addition( Polynomial &addend, const Polynomial adder )
{
   ...
   //stop once one of the arrays comes to end
    while(i<addend.size&&j<adder.size)
    {
        ...
        {
            sum.terms[k]=addend.terms[i];
            i++;
        }
        else if(addend.terms[i].expon<adder.terms[j].expon)
        {
            sum.terms[k]=adder.terms[j];
            j++;
        }
        else if(addend.terms[i].expon==adder.terms[j].expon)
        {
       ...
       for(int m=0; m<multiplier.size; m++)
        {
            ...
            {
                buffer.terms[n].expon=multiplier.terms[m].expon+multiplicand.terms[n].expon;
                buffer.terms[n].coef=multiplier.terms[m].coef*multiplicand.terms[n].coef;
            }
   ...
   while(remainder.size!=0&&remainder.terms[0].expon>=divisor.terms[0].expon)
    {
        ...
        monomial.terms[0] = temp.terms[ temp.size ];
