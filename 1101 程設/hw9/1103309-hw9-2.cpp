...
void readDictionary( string *dictionary, int &numDicWords )
{
    ...
    if (!inFile) {
        cout << "Failed to open the dictionary list" << endl;
        exit(1);
    }
                  ...
                  int numDicWords, string *result, int &numResult )
{
   ...
   int numWords = 0;
   string *words = 0;
   //strings to store same length words
...
void saveWords( string *result, int numResult )
{
    ofstream outFile("answers2.txt", ios::out);

    if (!outFile) {
        cout << "Failed to write into the result list" << endl;
        exit(1);
    }
