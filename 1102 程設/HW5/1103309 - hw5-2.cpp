...
bool HugeInteger< T >::operator<( const HugeInteger< T > &right ) const
{
    ...
    {
        for (size_type t = right.integer.size(); t > 0; )
        {
            t--;
...
void HugeInteger< T >::operator-=( HugeInteger< T > &op2 )
{
    ...
    for (size_type t = 0; t < difference.size(); t++)
    {
        difference.integer[t] += 10;
    }
    for (size_type t = 0; t < op2.size(); t++)
    {
        difference.integer[t] -= op2.integer[t];
    }
    for (size_type t = 0; t < difference.size(); t++)
    {
        if (difference.integer[t] < 10)
        {
            difference.integer[t + 1]--;
        }
        else
        {
            difference.integer[t] -= 10;
    ...
    for (size_type t = difference.size() - 1; t > 0; t--)
    {//delete excessive zeros
        if (difference.integer[t] == 0) {
...
void HugeInteger< T >::operator*=( HugeInteger< T > &op2 )
{
   ...
   if( isZero() || op2.isZero() )
   {
    ...
    for (size_type m = 0; m < integer.size(); m++)
    {
        for (size_type n = 0; n < op2.integer.size(); n++)
        {
            product.integer[m + n] += integer[m] * op2.integer[n];
        }
    }
    for (size_type t = 0; t < product.integer.size(); t++)
    {
        if (product.integer[t] >= 10)
        {
            product.integer[t + 1] += product.integer[t] / 10;
            product.integer[t] %= 10;
        }
    }
    for (size_type t = product.size() - 1; t > 0; t--)
    {
        if (product.integer[t] == 0)
        {
...
HugeInteger< T > HugeInteger< T >::operator/( HugeInteger< T > &op2 )
{
   ...
   HugeInteger<T> quotient(size() - op2.size() + 1);
   HugeInteger<T> remainder(*this);
   HugeInteger<T> buffer(size());

   for (size_type m = buffer.size(), n = op2.size(); m > 0 && n > 0; ) {
       m--, n--;
       buffer.integer[m] = op2.integer[n];
       if (n == 0) {
           break;
       }
   }
   ...
   for (size_type t = quotient.size(); t > 0; ) {
       t--;
